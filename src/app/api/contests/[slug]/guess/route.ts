...
import { submitContestGuess } from "@/lib/services/contests";
import { idOrSlugSchema } from "@/lib/validation/common";
import { contestGuessInputSchema } from "@/lib/validation/contests";
...
export async function POST(request: NextRequest, context: RouteContext): Promise<Response> {
  return handleApi(request, async ({ requestId }) => {
    ...
    const [{ slug }, input] = await Promise.all([
      context.params,
      parseJson(request, contestGuessInputSchema),
    ]);
    return apiJson(await submitContestGuess(idOrSlugSchema.parse(slug), input, actor, requestId));
  });
}
