...
import { createSubcategory, listAdminSubcategories } from "@/lib/services/admin-taxonomy";
import {
...
export async function GET(request: NextRequest): Promise<Response> {
  return handleApi(request, async () => {
    ...
    const query = parseSearchParams(request, adminSubcategoryQuerySchema);
    const result = await listAdminSubcategories(query);
    return apiList(result.subcategories, {
      limit: query.limit,
      offset: query.offset,
      total: result.total,
    });
...
export async function POST(request: NextRequest): Promise<Response> {
  return handleApi(request, async ({ requestId }) => {
    ...
    const input = await parseJson(request, subcategoryInputSchema);
    return apiJson(await createSubcategory(input, actor, requestId), { status: 201 });
  });
}
