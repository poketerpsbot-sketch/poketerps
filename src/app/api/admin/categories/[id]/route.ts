...
import { updateCategory } from "@/lib/services/admin-taxonomy";
import { updateCategorySchema } from "@/lib/validation/admin-management";
...
export async function PATCH(request: NextRequest, context: RouteContext): Promise<Response> {
  return handleApi(request, async ({ requestId }) => {
    ...
    return apiJson(await updateCategory(uuidSchema.parse(id), input, actor, requestId));
  });
}
