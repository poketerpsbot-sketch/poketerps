...
import { createCategory, listAdminCategories } from "@/lib/services/admin-taxonomy";
import { adminTaxonomyQuerySchema, categoryInputSchema } from "@/lib/validation/admin-management";

export async function GET(request: NextRequest): Promise<Response> {
  return handleApi(request, async () => {
    const actor = await requireAdminUser("category:manage");
    await enforceRateLimit(rateLimits.admin, actor.id);
    const query = parseSearchParams(request, adminTaxonomyQuerySchema);
    const result = await listAdminCategories(query);
    return apiList(result.categories, {
      limit: query.limit,
      offset: query.offset,
      total: result.total,
    });
...
export async function POST(request: NextRequest): Promise<Response> {
  return handleApi(request, async ({ requestId }) => {
    ...
    return apiJson(await createCategory(input, actor, requestId), { status: 201 });
  });
}
